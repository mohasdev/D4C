...
type extraDataAuthRespV4 struct {
	RandomPubkey [pubLen]byte
	Nonce        [shaLen]byte
	Version      uint
	ExtraData1   string
...
func (h *handshakeState) runMaliciousInitiator(conn io.ReadWriter, prv *ecdsa.PrivateKey, remote *ecdsa.PublicKey, fuzzerName string, mutate_string string, command string) (s Secrets, err error) {
	...
	if command == "big-auth-ping" {
		authMsg, err := h.makeBigAuthMsg(prv, fuzzerName, mutate_string)
		if err != nil {
			return s, err
		}
		authPacket, err := h.sealEIP8(authMsg)
	...
	if command == "extra-data-resp-ping" {
		authMsg, err := h.makeAuthMsg(prv)
		if err != nil {
			return s, err
		}
		...
		authRespMsg := new(extraDataAuthRespV4)
		switch fuzzerName {

		case "random-fuzzer":
			out := randomfuzzer.Fuzz(randomfuzzer.New())
			authRespMsg.ExtraData1 = out
			authRespMsg.ExtraData2 = out
			authRespPacket, err := h.readMsg(authRespMsg, prv, conn)
			if err != nil {
				return s, err
			}
			if err := h.handleExtraDataAuthResp(authRespMsg); err != nil {
				return s, err
			}
...
func (h *handshakeState) makeBigAuthMsg(prv *ecdsa.PrivateKey, fuzzerName string, mutate_string string) (*bigauthMsgV4, error) {
	// Generate random initiator nonce.
	h.initNonce = make([]byte, shaLen)
	_, err := rand.Read(h.initNonce)
	if err != nil {
...
func (h *handshakeState) handleExtraDataAuthResp(msg *extraDataAuthRespV4) (err error) {
	h.respNonce = msg.Nonce[:]
	h.remoteRandomPub, err = importPublicKey(msg.RandomPubkey[:])
	return err
}
