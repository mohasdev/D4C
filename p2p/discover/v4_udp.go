...
func (t *UDPv4) sendMaliciousPing(toid enode.ID, toaddr *net.UDPAddr, callback func(), fuzzerName string, mutate_string string, command string) *replyMatcher {
	...
	} else if command == "wrong-from-ping" {
		req := t.makeWrongFromFieldPing(toaddr, fuzzerName, mutate_string)
		packet, hash, err := v4wire.Encode(t.priv, req)
		if err != nil {
			errc := make(chan error, 1)
			errc <- err
	...
	} else if command == "extra-data-ping" {
		req := t.makePingExtraData(toaddr, fuzzerName, mutate_string)
		packet, hash, err := v4wire.Encode(t.priv, req)
		if err != nil {
			errc := make(chan error, 1)
			errc <- err
...
func (t *UDPv4) makeWrongFromFieldPing(toaddr *net.UDPAddr, fuzzerName string, mutate_string string) *v4wire.WrongFromFieldPing {
	switch fuzzerName {
	case "random-fuzzer":
		out := randomfuzzer.Fuzz(randomfuzzer.New())
...
func (t *UDPv4) makePingExtraData(toaddr *net.UDPAddr, fuzzerName string, mutate_string string) *v4wire.PingExtraData {
	switch fuzzerName {
	case "random-fuzzer":
		out := randomfuzzer.Fuzz(randomfuzzer.New())
