...
var (
	rlpxCommand = &cli.Command{
		...
		Subcommands: []*cli.Command{
			...
			rlpxEthTestCommand,
			rlpxSnapTestCommand,
			rlpxWrongAuthVersionCommand,
			rlpxWrongRespVersionCommand,
			rlpxBigAuthPingCommand,
			rlpxExtraDataAuthCommand,
			rlpxExtraDataRespCommand,
		},
	}
	...
	rlpxExtraDataAuthCommand = &cli.Command{
		Name:      "extra-data-ping",
		Usage:     "ping node with extra data in auth-body",
		ArgsUsage: "<node> <fuzzer-name> <run> <string-to-mutate>",
		Action:    rlpxExtraDataAuth,
	}
	rlpxExtraDataRespCommand = &cli.Command{
		Name:      "extra-data-resp-ping",
		Usage:     "ping node with extra data in ack-body",
		ArgsUsage: "<node> <fuzzer-name> <run> <string-to-mutate>",
		Action:    rlpxExtraDataResp,
	}
...
func rlpxWrongAuthVersion(ctx *cli.Context) error {
	var (
		n             = getNodeArg(ctx)
...
func rlpxExtraDataAuth(ctx *cli.Context) error {
	var (
		n             = getNodeArg(ctx)
		fuzzer_name   = ctx.Args().Get(1)
		run           = ctx.Args().Get(2)
...
func rlpxExtraDataResp(ctx *cli.Context) error {
	var (
		n             = getNodeArg(ctx)
		fuzzer_name   = ctx.Args().Get(1)
		run           = ctx.Args().Get(2)
